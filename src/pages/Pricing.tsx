  ...
  useEffect(() => {
    const fetchTiers = async () => {
      try {
        ...
        const tiers = tierSnapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() } as SubscriptionTier)
        );

        const formattedTiers = tiers.map((tier) => {
          const monthlyPlan = tier.plans.find(
            (plan) => plan.type === "monthly"
          );
          const lifetimePlan = tier.plans.find(
          ...
          return {
            ...
            stripeLifetimePriceId: lifetimePlan
              ? "lifetime_price_placeholder"
              : undefined,
          };
        });
